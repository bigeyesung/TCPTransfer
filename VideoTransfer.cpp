...
#include "IDEAI_VideoTransfer.h"
    ...
    async_tcp_client::async_tcp_client(boost::asio::io_service& io_service, const std::string& server, const std::string& path)
		: resolver_(io_service), socket_(io_service), m_acceptor(io_service, tcp::endpoint(tcp::v4(), 2222))
	{
	...
	/*	m_acceptor.async_accept(socket_,
			boost::bind(&async_tcp_client::accept_handler, this, socket_, _1));*/
